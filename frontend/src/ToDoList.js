...
function ToDoList({listId, handleBackButton}){
    ...
    function handleDeleteItem(id){
        const updateData = async () => {
            const response = await axios.delete(`/api/lists/${listId}/items/${id}`);
            setListData(await response.data);
        };
        updateData();
    }
    ...
    if (listData === null) {
        ...
        return (
            <div className="ToDoList">
                ...
                <div className="box">
                    <label htmlFor="new-item">New Item:&nbsp; 
                ...
                {listData.items.length > 0 ? (listData.items.map((item) => {
                    return (
                        <div key={item.id}
                    ...
                    <div className="box">No items in list!</div>
                )}
            </div>
        );
    }
